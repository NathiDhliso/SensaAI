...
import { Mail, Lock, LogIn, Loader2, ArrowRight, Eye, EyeOff, Sparkles, GraduationCap, Settings2 } from 'lucide-react';
...
export function Login() {
    ...
    const [loginRole, setLoginRole] = useState<'learner' | 'curator'>('learner');
    ...
    useEffect(() => {
        if (isAuthenticated) {
            ...
            if (!from || from === '/') {
                from = loginRole === 'curator' ? '/curator' : '/';
            } else if (loginRole === 'curator' && !from.startsWith('/curator')) {
                from = '/curator';
            }
            navigate(from, { replace: true });
        }
    }, [isAuthenticated, navigate, location, loginRole]);
            ...
            <div className={styles.glassCard}>
                ...
                <div className={styles.formSide}>
                    <div className={styles.header}>
                        ...
                        <p className={styles.subtitle}>Sign in to continue your {loginRole === 'learner' ? 'learning journey' : 'curation work'}</p>
                    </div>

                    <div className={styles.roleToggle}>
                        <button
                            type="button"
                            className={`${styles.roleBtn} ${loginRole === 'learner' ? styles.roleBtnActive : ''}`}
                        ...
                        <button
                            type="button"
                            className={`${styles.roleBtn} ${loginRole === 'curator' ? styles.roleBtnActive : ''}`}
                            onClick={() => setLoginRole('curator')}
                        >
                            <Settings2 size={16} />
                            Curator
                        </button>
