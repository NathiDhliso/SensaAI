...
import { Search, Archive, Sparkles, Clock, Zap, ChevronDown, ChevronUp, Target, Plus, X, GitBranch, Award, Wand2, Loader2, BookOpen, BarChart3, CheckCircle, Activity, ArrowRight } from 'lucide-react';
...
import type { SavedResult } from '@/features/content-storage';
...
export default function Home() {
    ...
    const loadDashboardData = useCallback(async () => {
        ...
        try {
            const results = await storageManager.listResults();
    ...
    const hasDashboard = isAuthenticated && savedResults.length > 0;
    ...
    const getDecayColor = (status: string) => {
        if (status === 'forgotten') return 'var(--color-error)';
        if (status === 'fading') return 'var(--color-warning)';
    ...
    const formatDueDate = (dateStr: string) => {
        ...
        return `Due in ${Math.abs(diffDays)}d`;
    };
    /* Derived State */
    ...
    if (hasDashboard) {
        const spacing = getSpacingEngine();
        const totalConcepts = spacingMetrics?.totalConcepts ?? 0;
        const dueCount = dueReviews.length;
        const retention = spacingMetrics?.retentionRate ?? 100;
        const healthPct = knowledgeHealth ?? 100;
        const visibleDue = dueReviews.slice(0, 5);
                ...
                <div className={styles.dashboard}>
                    <div className={styles.dashboardGreeting}>
                        <div className={styles.greetingText}>
                            <h1 className={styles.greetingTitle}>{getGreeting()}</h1>
                            <p className={styles.greetingSubtitle}>
                                {savedResults.length} subject{savedResults.length !== 1 ? 's' : ''} in your library
                            </p>
                        </div>
                    </div>
                    ...
                    <div className={styles.globalStats}>
                        <div className={styles.globalStat}>
                            <span className={styles.globalStatValue}>{savedResults.length}</span>
                            <span className={styles.globalStatLabel}>Subjects</span>
                        </div>
                        <div className={styles.globalStat}>
                            <span className={styles.globalStatValue}>{totalConcepts}</span>
                            <span className={styles.globalStatLabel}>Concepts</span>
                        </div>
                        <div className={styles.globalStat}>
                            <span className={styles.globalStatValue} style={{ color: dueCount > 0 ? 'var(--color-error)' : undefined }}>{dueCount}</span>
                            <span className={styles.globalStatLabel}>Due Reviews</span>
                        </div>
                        <div className={styles.globalStat}>
                            <span className={styles.globalStatValue} style={{ color: getHealthColor(healthPct) }}>{healthPct}%</span>
                            <span className={styles.globalStatLabel}>Health</span>
                        </div>
                    </div>

                    {dueCount > 0 && (
                        <div className={styles.dueSection}>
                            <div className={styles.dueSectionHeader}>
                                <h2 className={styles.dueSectionTitle}>
                                    <Activity size={16} />
                                    Due Reviews
                                    <span className={styles.dueSectionBadge}>{dueCount}</span>
                                </h2>
                                {dueCount > 5 && (
                                    <button className={styles.dueViewAll} onClick={() => navigate('/library')}>
                                        View all
                                    </button>
                                )}
                            </div>
                            <div className={styles.dueList}>
                                {visibleDue.map(review => {
                                    const decay = spacing.getDecayStatus(review.conceptId);
                                    return (
                                        <button
                                            key={review.id}
                                            className={styles.dueItem}
                                            onClick={() => {
                                                const result = savedResults.find(r =>
                                                    r.fullDocument?.includes(review.conceptId)
                                                );
                                                if (result) navigate(`/launchpad/${result.id}`);
                                            }}
                                        >
                                            <span className={styles.dueDecay} style={{ background: getDecayColor(decay) }} />
                                            <div className={styles.dueInfo}>
                                                <div className={styles.dueName}>{review.conceptName}</div>
                                                <div className={styles.dueMeta}>
                                                    <span>{formatDueDate(review.dueDate)}</span>
                    ...
                    <div className={styles.subjectsSection}>
                        <h2 className={styles.subjectsSectionTitle}>
                            <BookOpen size={16} />
                            Your Subjects
                        </h2>
                        <div className={styles.subjectsGrid}>
                            {savedResults.map(result => {
                                const conceptIds = result.pass1Data?.concepts ?? [];
                                const subjectDue = conceptIds.filter(cid => {
                                    const r = spacing.getReview(cid);
                                    return r && new Date(r.dueDate) <= new Date();
                                }).length;
                                const subjectFreshCount = conceptIds.filter(cid => spacing.getDecayStatus(cid) === 'fresh').length;
                                const subjectHealth = conceptIds.length > 0 ? Math.round((subjectFreshCount / conceptIds.length) * 100) : 100;

                                return (
                                    <div key={result.id} className={styles.subjectCard}>
                                        <div className={styles.subjectCardHeader}>
                                            <span className={styles.subjectCardName}>{result.subject}</span>
                                            {subjectDue > 0 && (
                                                <span className={styles.subjectCardDueBadge}>{subjectDue} due</span>
                                            )}
                                        </div>
                                        <div className={styles.subjectCardMeta}>
                                            <span>{conceptIds.length} concepts</span>
                                            <span>{subjectHealth}% health</span>
                                        </div>
                                        <div className={styles.subjectCardHealthBar}>
                                            <div
                                                className={styles.subjectCardHealthFill}
                                                style={{
                                                    width: `${subjectHealth}%`,
                                                    background: getHealthColor(subjectHealth)
                                                }}
                                            />
                                        </div>
                                        <div className={styles.subjectCardActions}>
                                            <button
                                                className={`${styles.subjectCardAction} ${styles.subjectCardActionPrimary}`}
                    ...
                    {canGenerate && (
                        <div className={styles.generateSection}>
                            ...
                            >
                                <Sparkles size={14} />
                                <span>Generate New Subject</span>
                                {showGenerateForm ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                            </button>
                            <AnimatePresence>
                                {showGenerateForm && (
                                    <motion.div
                                    ...
                                    >
                                        <div className={styles.generateSectionBody}>
                                            {selectedCert && (
                                                <div className={styles.certBadge}>
                                                    <Award size={14} />
                                                    <span>{selectedCert.name} ({selectedCert.code})</span>
                                            ...
                                            <div className={styles.inputSection}>
                                                <div className={styles.inputWrapper}>
                                                    ...
                                                    />
                                                    <AnimatePresence>
                                                        {showSuggestions && subject.trim().length >= 2 && !selectedCert && (
                                                            <motion.div
                                                                className={styles.unifiedDropdown}
                                                            ...
                                                            >
                                                                ...
                                                                <div className={styles.dropdownSection}>
                                                                    ...
                                                                    >
                                                                        {suggesting ? <Loader2 size={14} className={styles.spinning} /> : <Wand2 size={14} />}
                                                                        <span>{suggesting ? 'Analyzing...' : `AI analyze "${subject.trim().slice(0, 30)}${subject.trim().length > 30 ? '...' : ''}" structure`}</span>
                                                                    </button>
                                                                </div>
                                                            </motion.div>
                                                        )}
                                                            ...
                                                            <div className={styles.trunksList}>
                                                                {trunks.map((trunk, i) => (
                                                                    <div key={i} className={styles.trunkRow}>
                                                                        ...
                                                                        />
                                                                        {trunks.length > 2 && (
                                                                            <button type="button" onClick={() => removeTrunk(i)} className={styles.trunkRemove}>
                                                                                <X size={14} />
                                                                            </button>
                                                                        )}
                                                                    </div>
                                        ...
                                        <div className={styles.generateSectionFooter}>
                                            <div className={styles.groundingStatus}>
                                                {validTrunks.length >= 2 ? (
                                                    <div className={styles.groundedStatus}>
                                                        <GitBranch size={14} />
