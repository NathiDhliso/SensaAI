...
const LABEL_PRESETS = [
    'solid', 'dashed', 'arrow', 'double-arrow'
];
const RELATIONSHIP_LEGEND = [
    { id: 'solid', label: 'Solid Line', meaning: 'Is / Has / Belongs to', example: 'A direct, factual relationship. Parent-child, category-member, whole-part.' },
    { id: 'dashed', label: 'Dashed Line', meaning: 'Influences / Relates to', example: 'An indirect or associative relationship. Cross-branch, cause-effect.' },
    { id: 'arrow', label: 'Arrow (one-way)', meaning: 'Leads to / Requires', example: 'A directional relationship. Sequence, dependency, output.' },
    { id: 'double-arrow', label: 'Double Arrow', meaning: 'Depends Mutually', example: 'Bidirectional dependency or feedback loop.' }
];
const normalizeConnectionLabel = (label: string): string => {
    const normalized = label.toLowerCase().trim();
    if (normalized === 'is part of' || normalized === 'is-part-of' || normalized === 'has' || normalized === 'contains' || normalized === 'is') return 'solid';
    if (normalized === 'influences' || normalized === 'relates' || normalized === 'connects') return 'dashed';
    if (normalized === 'requires' || normalized === 'produces' || normalized === 'leads to' || normalized === 'enables' || normalized === 'causes') return 'arrow';
    if (normalized === 'mutually depends' || normalized === 'reinforces' || normalized === 'depends mutually') return 'double-arrow';
    if (['solid', 'dashed', 'arrow', 'double-arrow'].includes(normalized)) return normalized;
    // Fallbacks for old types
    if (normalized === 'depends-on' || normalized === 'depends_on' || normalized === 'prerequisite') return 'arrow';
    if (normalized === 'related-to' || normalized === 'related to' || normalized === 'is-type-of') return 'dashed';
    return label.trim();
};
// ============================================================================
// TYPES
...
export default function ConceptMapBuilder({
    ...
    const createLogicalConnections = useCallback((allNodes: MapNode[], existingConns: Connection[]): Connection[] => {
        ...
        if (dependencyGraph?.edges) {
            for (const edge of dependencyGraph.edges) {
                ...
                const label = edge.type === 'prerequisite' ? 'arrow' : 'dashed';
                newConns.push({
    ...
    const renderConnections = () => {
        return connections.map(conn => {
            ...
            const type = normalizeConnectionLabel(conn.label);
            if (type === 'dashed') strokeDasharray = '5,5';
            else if (type === 'arrow') markerEnd = 'url(#arrowhead)';
            else if (type === 'double-arrow') {
                markerEnd = 'url(#arrowhead)';
                markerStart = 'url(#arrowhead-start)';
            } else if (type !== 'solid') {
                strokeDasharray = '5,5'; // custom labels default to dashed
            }
            ...
            return (
                ...
                >
                    <line
                        ...
                        style={{ transition: 'all 0.2s', opacity: isSelected ? 1 : 0.7 }}
                    />
    ...
    const renderConnectionLabels = () => {
        return connections.map(conn => {
            ...
            const isSelected = selectedConnectionId === conn.id;
            const isStandardRate = ['solid', 'dashed', 'arrow', 'double-arrow'].includes(normalizeConnectionLabel(conn.label));

            // HIDE text for standard RATE types unless selected/editing
            if (isStandardRate && !isEditing && !isSelected) return null;
