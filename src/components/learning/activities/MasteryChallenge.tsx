    ...
    for (const concept of pool) {
        ...
        if (concept.howToUse && concept.howToUse.length > 0) {
            ...
            questions.push({
                ...
                correct: step,
                options: shuffle([step, ...distractorSteps.slice(0, 3)])
            });
        }
    }
...
export default function MasteryChallenge({ concepts, onComplete }: MasteryChallengeProps) {
    ...
    const finalScore = useMemo(() => {
        ...
        // Mastery Grade: S (≥90%) / A (≥75%) / B (≥60%) / C (≥40%) / D (<40%)
        const overallPct = Math.round(overall * 100);
        const grade = overallPct >= 90 ? 'S' : overallPct >= 75 ? 'A' : overallPct >= 60 ? 'B' : overallPct >= 40 ? 'C' : 'D';
        // Actionable tips based on breakdown
        const tips: string[] = [];
        if (recallScore < 0.5) tips.push('Review individual concept definitions — try the Blank Sheet Test for each concept you missed.');
        if (recallScore >= 0.5 && recallScore < 0.8) tips.push('Your recall is decent but not rock-solid — one more pass through the Learn phase will cement it.');
        if (connScore < 0.5 && hasConnections) tips.push('Concept connections need work — revisit the Concept Map and trace how concepts depend on each other.');
        if (synthesis.score < 0.5) tips.push('Your synthesis missed key concepts — try writing out how each concept connects before combining them.');
        if (synthesis.missed.length > 0 && synthesis.missed.length <= 3) tips.push(`Specifically revisit: ${synthesis.missed.join(', ')}.`);
        if (overallPct >= 90) tips.push('Exceptional mastery — consider moving to harder concepts or teaching this to solidify.');
        if (overallPct >= 75 && overallPct < 90) tips.push('Strong performance — one more session with interleaved practice will push you to S-grade.');
        return {
            recallCorrect,
            recallTotal,
                    ...
                    {round === 'results' && finalScore && (
                        ...
                        >
                            <div className={`${styles.celebrationCard} ${finalScore.passed ? '' : styles.celebrationFailed}`}>
                                ...
                                <div className={`${styles.gradeBadge} ${styles['grade' + finalScore.grade]}`}>
                                    <span className={styles.gradeLabel}>Grade</span>
                                    <span className={styles.gradeLetter}>{finalScore.grade}</span>
                                </div>
                                <h2>{finalScore.grade === 'S' ? 'Exceptional!' : finalScore.grade === 'A' ? 'Outstanding!' : finalScore.passed ? 'Well Done!' : 'Keep Practicing'}</h2>
