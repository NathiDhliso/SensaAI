...
import { getFoundationConcepts } from '@/features/learning-session/algorithms/concept-selection';
...
const ConceptChip = ({ concept, className, isFoundation }: { concept: LearningConcept; className: string; isFoundation?: boolean }) => {
    const adaptedContent = useMetaphorContent(concept);
    return (
        <motion.div
            className={`${className} ${isFoundation ? styles.foundationChip : ''}`}
        ...
        >
            ...
            {isFoundation && <span className={styles.foundationBadge}>★</span>}
        </motion.div>
    );
};
// ============================================================================
                    ...
                    <div className={styles.tierConceptList}>
                        {conceptsByTier.trunk.map(c => (
                            <ConceptChip
                                ...
                                isFoundation={foundationIds.has(c.id)}
                            />
                        ))}
                    </div>
                </div>
