        ...
        async (result: GenerationResult) => {
            ...
            if (currentPass1 && currentValidation) {
                ...
                try {
                    ...
                    const targetRoute = isCuratorMode ? `/curator/preview/${resultId}` : `/launchpad/${resultId}`;
                ...
                const targetRoute = window.location.pathname.startsWith('/curator')
                    ? `/curator/preview/${fallbackId}`
                    : `/launchpad/${fallbackId}`;
