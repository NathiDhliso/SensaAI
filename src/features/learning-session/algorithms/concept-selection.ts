...
// ============================================================================
// CRITICAL PATH / FOUNDATION CONCEPTS
// ============================================================================
/**
 * Get the top N "foundation" concepts by outdegree — concepts that unlock
 * the most other concepts. Used by Enhancement B (Critical Path Visualisation).
 */
export function getFoundationConcepts(
    concepts: LearningConcept[],
    topN = 5
): LearningConcept[] {
    return [...concepts]
        .sort((a, b) => {
            const aOut = (a.outdegree ?? 0) + (a.connections?.filter(c => c.type === 'enables').length ?? 0);
            const bOut = (b.outdegree ?? 0) + (b.connections?.filter(c => c.type === 'enables').length ?? 0);
            return bOut - aOut;
        })
        .slice(0, topN);
}
