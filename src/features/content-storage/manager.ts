    ...
    */
    ...
    async loadResult(id: string): Promise<SavedResult | null> {
        try {
            ...
            // 3. Reconstruct the document (pass classification so deepStructure is preserved)
            const classificationRecord = jobStatus.classification as Record<string, unknown> | undefined;
            const fullDocument = buildDocumentFromConcepts(resolvedSubject, allConcepts, classificationRecord);
