...
function parseDomainAnalysis(content: string): ParsedDomainAnalysis {
       ...
       let deepStructure: import('@/shared/types/generation').DeepStructure | undefined;
       let lifecycleBlueprints: { phase1: import('@/shared/types/generation').LifecycleBlueprint; phase2: import('@/shared/types/generation').LifecycleBlueprint | null; phase3: import('@/shared/types/generation').LifecycleBlueprint | null } | undefined;
       let examDomains: { name: string; weight: number; subtopics: string[] }[] | undefined;
       try {
              let cleanContent = content.replace(/^\uFEFF/, '');
              // eslint-disable-next-line no-control-regex
              cleanContent = cleanContent.replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F-\u009F]/g, '');
              ...
              if (firstBrace !== -1) {
                     ...
                     if (parsed.deepStructure && typeof parsed.deepStructure === 'object') {
                            deepStructure = parsed.deepStructure;
                     }
                     if (parsed.lifecycleBlueprints && typeof parsed.lifecycleBlueprints === 'object') {
                            lifecycleBlueprints = parsed.lifecycleBlueprints;
                     }
                     if (Array.isArray(parsed.examDomains)) {
                            examDomains = parsed.examDomains;
                     }
                     // If we have the new deep structure fields, reconstruct the full SubjectClassification
                     if (deepStructure || lifecycleBlueprints || examDomains) {
                            classification = {
                                   subjectType: subjectType || 'conceptual',
                                   classification: classification as { type: string; label: string; goal: string } || { type: subjectType || 'conceptual', label: '', goal: '' },
                                   deepStructure: deepStructure!,
                                   lifecycleBlueprints: lifecycleBlueprints!,
                                   examDomains: examDomains || [],
                                   connectiveTissue,
                                   lifecycle: lifecycle,
                            } as import('@/shared/types/macro-workflow').SubjectClassification;
                     }
              }
       } catch {
